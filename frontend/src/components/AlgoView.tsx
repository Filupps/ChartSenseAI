...
export const AlgoView: React.FC<AlgoViewProps> = ({ prediction, editable = false, onAlgorithmUpdate }) => {
  ...
  const generateWithLLM = async (type: 'pseudocode' | 'python') => {
    ...
    const prompt = type === 'pseudocode' 
      ? `Ты получаешь JSON алгоритма, извлечённого из диаграммы с помощью OCR.
Текст в шагах может содержать артефакты OCR: лишние символы (l, v, ~, >, AJ, bt, ый, т и т.д.), 
искажённые слова, случайные буквы в начале/конце, слипшиеся или разорванные слова.

Твоя задача:
1. Восстановить исходный смысл каждого шага, исправив OCR-артефакты.
   Примеры типичных искажений: "l расчещетоймости" → "Расчёт стоимости", 
   "Саяраьыть JSON npoerta в" → "Собрать JSON проекта", "W6pstons werpwens" → "Обработка материалов",
   "подготовка bt" → "Подготовка", "ый Глубокий онолиз" → "Глубокий анализ".
2. Преобразовать алгоритм в чистый псевдокод на русском языке.
3. Использовать стандартные конструкции: НАЧАЛО, КОНЕЦ, ЕСЛИ-ТО-ИНАЧЕ, ПОКА, ДЛЯ.

Выведи ТОЛЬКО исправленный псевдокод без пояснений и без markdown-обёрток.
...
1. Восстановить исходный смысл каждого шага, исправив OCR-артефакты.
   Примеры типичных искажений: "l расчещетоймости" → "Расчёт стоимости",
   "Саяраьыть JSON npoerta в" → "Собрать JSON проекта", "W6pstons werpwens" → "Обработка материалов".
2. Преобразовать алгоритм в работающий Python код.
3. Создать функцию process_algorithm(), которая реализует логику алгоритма.
4. Добавить комментарии на русском языке с исправленными названиями шагов.

Выведи ТОЛЬКО код без пояснений и без markdown-обёрток.
